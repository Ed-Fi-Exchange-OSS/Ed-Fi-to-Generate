...
SELECT @EdFiCount = Count(*) FROM (
			SELECT DISTINCT
				...
				,0			StudentImmigrant			-- by default student is not immigrant
				,langc.CodeValue LanguageInstructionProgramServiceType
				,NULL		StudentImmigrant_StartDate
				,NULL		StudentImmigrant_EndDate
				,NULL PersonID
...
FROM ODS.ProgramParticipationTitleIIILep titleiii
INNER JOIN Staging.ProgramParticipationTitleIII ppi
ON titleiii.[PersonProgramParticipationId] = ppi.[PersonProgramParticipationId]
