	...
	BEGIN
		...
		*/
		...
		SELECT 
			...
			, NULL RunDateTime
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateEducationAgency sea
		...
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationAddress edorgadd
			ON edorg.EducationOrganizationId = edorgadd.EducationOrganizationid
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].AddressType aty
			ON edorgadd.AddressTypeId = aty.AddressTypeId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateAbbreviationType sat
		...
		SELECT DISTINCT
			...
			,NULL RunDateTime
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].School sch
		...
		LEFT JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].SchoolType st ON sch.SchoolTypeId = st.SchoolTypeId
		LEFT JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].OperationalStatusType ostlea ON edorglea.OperationalStatusTypeId = ostlea.OperationalStatusTypeId
		LEFT JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].OperationalStatusType ostschool ON edorgschool.OperationalStatusTypeId = ostschool.OperationalStatusTypeId
		LEFT JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].CharterStatusType leacst ON lea.CharterStatusTypeId = leacst.CharterStatusTypeId
		LEFT JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].CharterStatusType schcst ON sch.CharterStatusTypeId = schcst.CharterStatusTypeId
		-- LEA NCES IDs
		LEFT JOIN (
			SELECT 
				  edorgidcodelea.EducationOrganizationId
				, edorgidcodelea.IdentificationCode
			FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationCode edorgidcodelea 
			JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationSystemDescriptor edorgidsdlea ON edorgidcodelea.EducationOrganizationIdentificationSystemDescriptorId = edorgidsdlea.EducationOrganizationIdentificationSystemDescriptorId
			JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationSystemType edorgidstlea ON edorgidsdlea.EducationOrganizationIdentificationSystemTypeId = edorgidstlea.EducationOrganizationIdentificationSystemTypeId 
		...
		LEFT JOIN (
			...
			FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationCode edorgidcodesch 
			JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationSystemDescriptor edorgidsdsch ON edorgidcodesch.EducationOrganizationIdentificationSystemDescriptorId = edorgidsdsch.EducationOrganizationIdentificationSystemDescriptorId
			JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationIdentificationSystemType edorgidstsch ON edorgidsdsch.EducationOrganizationIdentificationSystemTypeId = edorgidstsch.EducationOrganizationIdentificationSystemTypeId
		...
		SELECT DISTINCT
			...
			,NULL RunDateTime
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateEducationAgency sea
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorgsea ON sea.StateEducationAgencyId = edorgsea.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationAddress edorgaddress ON edorgsea.EducationOrganizationId = edorgaddress.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].AddressType aty ON edorgaddress.AddressTypeId = aty.AddressTypeId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateAbbreviationType sat ON edorgaddress.StateAbbreviationTypeId = sat.StateAbbreviationTypeId
		...
		SELECT DISTINCT
			...
			,NULL LocationId
			,NULL RunDateTime
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].LocalEducationAgency lea
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorglea ON lea.LocalEducationAgencyId = edorglea.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationAddress edorgaddress ON edorglea.EducationOrganizationId = edorgaddress.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].AddressType aty ON edorgaddress.AddressTypeId = aty.AddressTypeId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateAbbreviationType sat ON edorgaddress.StateAbbreviationTypeId = sat.StateAbbreviationTypeId
		JOIN ODS.RefState sd ON sat.CodeValue = sd.Code
		WHERE edorgaddress.StreetNumberName IS NOT NULL
		...
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].School sch
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorgschool ON sch.SchoolId = edorgschool.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationAddress edorgaddress ON edorgschool.EducationOrganizationId = edorgaddress.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].AddressType aty ON edorgaddress.AddressTypeId = aty.AddressTypeId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateAbbreviationType sat ON edorgaddress.StateAbbreviationTypeId = sat.StateAbbreviationTypeId
		JOIN ODS.RefState sd ON sat.CodeValue = sd.Code
		WHERE edorgaddress.StreetNumberName IS NOT NULL
		...
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].StateEducationAgency sea
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorgsea ON sea.StateEducationAgencyId = edorgsea.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationInstitutionTelephone edorgphone ON edorgsea.EducationOrganizationId = edorgphone.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].InstitutionTelephoneNumberType itnt ON edorgphone.InstitutionTelephoneNumberTypeId = itnt.InstitutionTelephoneNumberTypeId
		...
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].LocalEducationAgency lea
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorglea ON lea.LocalEducationAgencyId = edorglea.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationInstitutionTelephone edorgphone ON edorglea.EducationOrganizationId = edorgphone.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].InstitutionTelephoneNumberType itnt ON edorgphone.InstitutionTelephoneNumberTypeId = itnt.InstitutionTelephoneNumberTypeId
		WHERE edorgphone.TelephoneNumber IS NOT NULL
		AND edorgphone.TelephoneNumber <> ''
		AND edorgphone.InstitutionTelephoneNumberTypeId IS NOT NULL
		...
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].School sch
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorgschool ON sch.SchoolId = edorgschool.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganizationInstitutionTelephone edorgphone ON edorgschool.EducationOrganizationId = edorgphone.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].InstitutionTelephoneNumberType itnt ON edorgphone.InstitutionTelephoneNumberTypeId = itnt.InstitutionTelephoneNumberTypeId
		WHERE edorgphone.TelephoneNumber IS NOT NULL
		AND edorgphone.TelephoneNumber <> ''
		AND edorgphone.InstitutionTelephoneNumberTypeId IS NOT NULL
		...
		FROM [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].School sch
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].EducationOrganization edorgschool ON sch.SchoolId = edorgschool.EducationOrganizationId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].SchoolGradeLevel sgl ON sch.SchoolId = sgl.SchoolId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].GradeLevelDescriptor gld ON sgl.GradeLevelDescriptorId = gld.GradeLevelDescriptorId
		JOIN [EdFi20StagingDB1].[EdFi_Ods_Generate].[edfi].GradeLevelType glt ON gld.GradeLevelTypeId = glt.GradeLevelTypeId
		--Ensure the OrganizationIdentifier is the same as the Staging.Organization table
		...
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.StateDetail'

		--Validate Staging.Organization
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.Organization'

		--Validate Staging.OrganizationAddress
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.OrganizationAddress'

		--Validate Staging.OrganizationGradeOffered
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.OrganizationGradeOffered'

		--Validate Staging.OrganizationPhone
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.OrganizationPhone'

		--Validate Staging.OrganizationFederalFunding
		EXEC App.Migrate_Data_Validation 'Migrate_Data_ETL_IMPLEMENTATION_STEP01_Organization', 'Staging.OrganizationFederalFunding'
